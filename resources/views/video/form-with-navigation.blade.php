...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-red-50 rounded-2xl p-6 mb-6">
        <div class="text-center">
            <h2 class="text-xl font-bold text-red-800 mb-2 leading-relaxed">{{ $title }}</h2>
            <p class="text-red-600">
                Surah: {{ $surahs[$currentSurah]['name'] }} ({{ $surahs[$currentSurah]['pashto'] }}) - 
                Verse: {{ $currentAyah }} of {{ $maxAyahs }}
            </p>
            <div class="w-full bg-red-200 rounded-full h-3 mt-4">
    ...
    <div class="bg-white rounded-2xl shadow-xl p-8">
        <form action="{{ route('video.save', $type) }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    ...
                    <div><strong>Type:</strong> Video File</div>
                </div>
            </div>

            <!-- Video Link -->
            <div>
                <label for="video_link" class="block text-lg font-semibold text-gray-700 mb-3">Video File Link:</label>
            ...
            <div class="flex flex-col sm:flex-row gap-4 pt-4">
                ...
                @if($currentAyah < $maxAyahs || $currentSurah < 114)
                    <button type="submit" name="action" value="next"
                            class="flex-1 bg-gradient-to-r from-red-500 to-orange-500 hover:from-red-600 hover:to-orange-600 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 flex items-center justify-center">
                        Next Verse
    ...
    <div class="bg-white rounded-2xl shadow-xl p-6 mt-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-2">
            ...
            @if($startVerse > 1)
                <a href="{{ route('video.form.ayah', [$type, $currentSurah, 1]) }}" 
                   class="p-2 text-center rounded-lg border-2 transition-all duration-200 bg-gray-50 hover:bg-red-50 border-gray-200 hover:border-red-300">
                    1
                </a>
                @if($startVerse > 2)
            ...
            @for($i = $startVerse; $i <= $endVerse; $i++)
                <a href="{{ route('video.form.ayah', [$type, $currentSurah, $i]) }}" 
                   class="p-2 text-center rounded-lg border-2 transition-all duration-200 {{ $i == $currentAyah ? 'bg-red-500 text-white border-red-500' : 'bg-gray-50 hover:bg-red-50 border-gray-200 hover:border-red-300' }}">
                    {{ $i }}
                </a>
            @endfor
            ...
            @if($endVerse < $maxAyahs)
                @if($endVerse < $maxAyahs - 1)
                    <div class="p-2 text-center text-gray-500">...</div>
                @endif
                <a href="{{ route('video.form.ayah', [$type, $currentSurah, $maxAyahs]) }}" 
                   class="p-2 text-center rounded-lg border-2 transition-all duration-200 bg-gray-50 hover:bg-red-50 border-gray-200 hover:border-red-300">
                    {{ $maxAyahs }}
                </a>
            @endif
